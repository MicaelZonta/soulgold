...
#define OW_FOLLOWERS_UNIQUE_SPRITES    TRUE      // If FALSE, every species uses the generic substitute overworld sprite instead of its own follower graphics/palette. Saves a large amount of ROM (no per-species follower sheets are compiled in).
#define OW_SURF_UNIQUE_SPRITES         FALSE      // If FALSE, Surfing always uses the generic surf blob instead of a per-species surf mount sprite. Saves a large amount of ROM (no per-species surf sheets are compiled in).
...
#define OW_BATTLE_ONLY_FORMS_NECROZMA_ULTRA TRUE  // SoulGold: per-species exception to OW_BATTLE_ONLY_FORMS. If TRUE, Ultra Necrozma keeps its overworld sprite (OBJ_EVENT_GFX_SPECIES(NECROZMA_ULTRA)) even while OW_BATTLE_ONLY_FORMS is FALSE, because the story uses it. Requires OW_POKEMON_OBJECT_EVENTS and P_ULTRA_BURST_FORMS.
