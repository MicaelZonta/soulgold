...
Revisão V25: 25 de setembro de 2026. Substitui o V24 apenas no **clímax**: o evento do **Altar do Sol e da Lua** deixa de ser "implementado, runtime pendente" e passa a **testado pelo autor**, com as cinco correções do retorno dele já no código ([`ALTAR_SUN_MOON_IMPLEMENTATION.md`](ALTAR_SUN_MOON/ALTAR_SUN_MOON_IMPLEMENTATION.md) §18). Três decisões saem daqui e valem para o arco inteiro: (1) **a captura do lendário é cena**, não caixa de texto — `enter_pokeball` no objeto da criatura, a Ball do personagem como objeto próprio, chacoalhada e clique em som, fanfarra no clique e só então o `givemon`; (2) **quem fica morando num lugar no pós-game se mexe** — `MOVEMENT_TYPE_WANDER_AROUND` trocado no `ON_TRANSITION` a partir do estado final, nunca durante as cenas que posicionam o elenco por coordenada; (3) **nome de `MAPSEC` novo tem no máximo 16 caracteres**, porque `GetMapName()` copia sem limite e acima de 19 ele corrompe memória (o "Altar of Sun and Moon", com 21, escrevia em cima de um ponteiro de função e travava o Town Map). Continua aberto um travamento de tela branca na fenda diária, com a investigação registrada no §18.5 do doc do altar.
...
Revisão V27: 25 de setembro de 2026. Substitui o V26 apenas no pós-E4: o roteiro `MAHOGANY_ULTRABEAST_SCRIPT_V2.md` (V3) foi aplicado inteiro à **Missão 2**. A rua sul de Mahogany foi **alargada em duas linhas** no `map.bin` (a cena antiga espremia duas UBs de 32×32, o Necrozma, o Mamoswine e quatro treinadores em três linhas); a barreira de gelo virou **arte de verdade** (quatro metatiles novos no secundário de Mahogany, com estado rompido); o circuito infinito virou **uma recarga com origem e custo**; o Pryce **cobre** em vez de atacar em vão; a Lillie corrige a própria hipótese **sem se depreciar**; o Necrozma recolhe as UBs **por cima do gelo** e a Anabel fica com **as duas assinaturas**, que é a primeira prova de que as UBs podem estar vivas; o parceiro da família Cosmog vira **ator**; e o retry ganha **dois formatos** com checkpoint persistente. Registro em §24; implementação em [`MAHOGANY_ULTRABEAST_IMPLEMENTATION.md`](MAHOGANY_ULTRABEAST/MAHOGANY_ULTRABEAST_IMPLEMENTATION.md) §16.
...
| Ligação do Looker + escritório de Olivine + Blackthorn (Missão 1) | **Roteiro V2 implementado (25/09/2026, V26):** o roteiro `BLACKTHORN_ULTRABEAST_SCRIPT_V2.md` substituiu todas as falas da ligação, do briefing e da cena; o ataque da Clair passa a **ter efeito visível** (o Necrozma recua um tile e volta); a fenda ganhou **sprite próprio** (`OBJ_EVENT_GFX_ALTAR_RIFT` em (14,51)); as duas UBs avançam com **ritmos e linhas diferentes** (nada de mente coletiva); o **Gladion nomeia o Necrozma** em cena; a Anabel **tenta conter** a UB derrotada e a Ball é repelida pela ligação; a oscilação da fenda depois da absorção é **obrigatória em todos os ramos**; o parceiro da família Cosmog **aparece como ator** em (19,51) antes de qualquer fala sobre ele; o Type: Null do presente é um **objeto próprio** ao lado do Silvally; o retry tem **texto próprio e checkpoint persistente** (`FLAG_BLACKTHORN_UB_ENGAGED`); a falha de entrega do presente **não repete a batalha** (`FLAG_BLACKTHORN_TYPE_NULL_PENDING`); trilha de ameaça implementada. Build limpo; **runtime pendente**. Números do boss inalterados. **Antes: história evoluída (22/09/2026, V19):** Clair contra Necrozma na chegada, UBs atacando o jogador, resgate do Gladion + Silvally, absorção pelo Necrozma, reação opcional à família Cosmog, presente do Type: Null com checagem de espaço antes do SIM, portas trancadas exceto o Centro, boss 3 barras / Lv75 / x120 / moveset curado + item. Falas finais. Build limpo; **validado em runtime pelo autor (22/09/2026)**. Receita na skill `evoluir-historia-de-evento`. **Antes: esqueleto implementado (19/09/2026):** conforme [`.claude/rift_missions/BLACKTHORN_ULTRABEAST/BLACKTHORN_ULTRABEAST_IMPLEMENTATION.md`](BLACKTHORN_ULTRABEAST/BLACKTHORN_ULTRABEAST_IMPLEMENTATION.md), da ligação ao gancho para a Missão 2 (Looker e Anabel moram em Olivine desde o New Game; escolha da UB + boss battle). Build limpo; runtime pendente. Primeira batalha de ameaça: derrota = blackout/retry. |
| Limpeza de treinadores | Informada como concluída pelo autor; este documento não certifica IDs ou contagens livres. |
| Missão 2 — Mahogany (Xurkitree + Celesteela, Lillie) | **Roteiro V3 implementado (25/09/2026, V27):** rua sul alargada de 3 para 5 linhas no `map.bin`, cena reorganizada em três áreas (abrigo/rua/apoio) com **dois enquadramentos de câmera**; barreira de gelo como **metatile real** (`METATILE_MahoganyTown_IceWall_*`), intransponível, rompível e removida em cena; **uma** recarga, com a fonte perdendo brilho ao enviar; Pryce cobrindo a porta do Ginásio em vez de um Blizzard inútil; Lillie corrigindo o plano em uma linha, sem pedir desculpas; Necrozma bloqueado no chão e alcançando as UBs **por cima do gelo**; **duas assinaturas persistem** no instrumento depois da absorção (a descoberta, independente da party); parceiro Cosmog/Cosmoem/Solgaleo/Lunala como **ator** em (17,23); Pryce cumprindo a promessa da lâmpada e saindo da cidade; Lillie ficando até os moradores voltarem (`FLAG_MAHOGANY_UB_LILLIE_SETTLING`); **dois retries** distintos (`FLAG_MAHOGANY_UB_ENGAGED` + `FLAG_MAHOGANY_UB_SAW_RECHARGE`), com a parede rompida por impacto físico; briefing de Olivine em quatro caixas reconhecendo Blackthorn V3; trilha de ameaça; nenhum texto citando hora do dia. Build limpo; **runtime pendente**. Números do boss inalterados. **Antes: história evoluída (22/09/2026, V20):** Lillie como descoberta ao chegar (fora do briefing); Pryce + Mamoswine evacuando os últimos moradores na tela, Blizzard sem efeito no Necrozma, entrada no Ginásio no fim; sinergia das UBs mostrada, vencendo a rodada 1 (revive) e derrotada pela parede de gelo da Lillie + Pryce; duas rodadas seguidas (2 barras, depois 4 / Lv80 / x130) com cura da Anabel entre elas; absorção pelo Necrozma; reação opcional à família Cosmog; portas trancadas exceto o Centro; gancho sem destino. Falas finais. Build limpo; **runtime pendente**. **Antes: esqueleto implementado (19/09/2026)**, conforme [`.claude/rift_missions/MAHOGANY_ULTRABEAST/MAHOGANY_ULTRABEAST_IMPLEMENTATION.md`](MAHOGANY_ULTRABEAST/MAHOGANY_ULTRABEAST_IMPLEMENTATION.md) §12. |
...
| Evento ALTAR DO SOL E DA LUA — Lusamine e Ultra Necrozma (clímax) | **Implementado e TESTADO EM RUNTIME pelo autor (25/09/2026, V25 — revisão 4):** as cinco correções do retorno dele estão no §18 do doc (crash do Town Map por nome de `MAPSEC` de 21 caracteres; fenda com sprite próprio 32x32 animado; Looker e Anabel andando no pós-game; presente que anunciava "SOLGALEO" e captura que não acontecia na tela; tela branca na fenda diária ainda em aberto, investigação no §18.5). **Revisão 5 (25/09/2026), §19:** o Ultra Necrozma da arena desenhava o boneco do Substitute — o `OVERWORLD(...)` da espécie estava sob `OW_BATTLE_ONLY_FORMS FALSE`. Resolvido com a feature flag `OW_BATTLE_ONLY_FORMS_NECROZMA_ULTRA` (exceção só dele, +8 bytes de ROM); nenhum mapa ou script mudou. **Revisão 6 (25/09/2026), §20:** continuidade com New Bark — a criatura sai de lá dourada, então na arena ela **chega** Ultra e só perde a luz ao ser derrotada. A transformação do meio da cena saiu; o compasso do parceiro herdou o lugar dela (flare, grito, shake). Só o `scripts.inc` da arena mudou. **Antes — planejado, revisão 2 (23/09/2026, V24):** conforme [`.claude/rift_missions/ALTAR_SUN_MOON/ALTAR_SUN_MOON_IMPLEMENTATION.md`](ALTAR_SUN_MOON/ALTAR_SUN_MOON_IMPLEMENTATION.md). **Nada implementado**, e o plano já traz a **história completa** em vez de placeholder, porque o terreno estava pronto (mapa, tileset com estado de portal, navio, arena, sprites, `MAPSEC` renomeada). Cinco atos, estados 12→13→14→15→16, num mapa e meio: chegada e oferta da Lusamine; duelo narrativo dela (SIM/NÃO, sem blackout, os dois resultados resolvem, a Lillie fecha); a fenda de chão no pátio; Ultra Necrozma na arena com a Anabel, boss de **5 barras / Lv90 / x160** e perfil de fases que **descasca** a criatura; captura roteirizada; despedida. **Fecha cinco pendências do §14** (Escritório pós-história, Beast Balls, Altar menos o Fly, Necrozma, Despedida) e é o **primeiro evento do arco sem flag persistente nova** — toda visibilidade é `FLAG_TEMP_*` recalculada, mais cinco daily flags. Deixa a fenda diária e a arena vazia como handoff para o documento do **loop**. |
...
| 2 | Xurkitree + Celesteela (+ Necrozma, que absorve as duas) | Mahogany | Lillie (+ Ninetales), descoberta ao chegar; **Pryce** (+ Mamoswine), Líder de Ginásio evacuando a cidade; Anabel presente | **Roteiro V3 implementado** (25/09/2026, V27), build limpo, runtime pendente | [`.claude/rift_missions/MAHOGANY_ULTRABEAST/MAHOGANY_ULTRABEAST_IMPLEMENTATION.md`](MAHOGANY_ULTRABEAST/MAHOGANY_ULTRABEAST_IMPLEMENTATION.md) |
...
**Refinamento da V27 (Mahogany).** A sinergia não pode ser um **circuito inesgotável**: a primeira versão de Mahogany dizia que uma drenava e a outra queimava e devolvia, para sempre, e isso tira o sentido de lutar. A forma correta é **uma transferência, com origem e custo**: quem envia **perde brilho ou reduz atividade** na mesma cena, a ruptura oscila, e a recuperação não é chamada de ressurreição. E a contramedida corta **aquele trajeto visível** — não é uma lei da natureza. Em Mahogany a parede de gelo interrompe a ligação que a tela mostrou, e o Necrozma depois cria outra por cima dela: o primeiro plano não foi inútil, a geometria do problema é que mudou.

**Forma da regra, a partir da V21** — em todo encontro daqui para a frente a sinergia precisa de três coisas: (a) ser **mostrada** com movimento/flash antes de qualquer explicação; (b) ter **consequência mecânica ou visual** que o jogador sinta (a M2 revive a derrotada; a M3 teleporta a pesada para o lado do jogador); (c) ter uma **contramedida que o acompanhante enxerga** e que justifique como o grupo se divide. Cada missão escolhe sua forma; não é obrigatório repetir as duas rodadas da M2. **M2, M3 e M4 cumprem; a M1 está de fora por decisão, não por atraso (V26):** o roteiro V2 manda explicitamente que as duas UBs de Blackthorn avancem com **ritmos e linhas diferentes** — o Buzzwole em três passos pesados pela linha reta, a Pheromosa com uma pausa, um amago para o lado e uma corrida mais rápida pela lateral — justamente para que a primeira missão **não** estabeleça mente coletiva. A sinergia é uma revelação do arco (V22: a criatura é que sincronizava as nove), e ela só funciona se a primeira cena mostrar duas criaturas soltas.
...
- **Espaço de cena é medido pelo sprite, não pela âncora (V27, aprendido em Mahogany).** Ultra Beasts, Necrozma e os parceiros de overworld são sprites de **32×32**: ocupam duas colunas e duas linhas de tela a partir do tile em que estão. Dois deles em tiles vizinhos se encavalam, e um Pokémon grande colado num treinador o **apaga** (em Mahogany o Mamoswine sumiu com o Pryce, e o parceiro do jogador virou uma mancha só com ele, a Lillie e a Ninetales). Regras que saíram disso: pelo menos **três tiles livres** entre as âncoras de duas UBs, **dois** entre cada frente e seu treinador, corredores separados para apoio e cura, e **renderizar o quadro antes de aceitar a formação** — o build nunca reclama de sprite sobreposto. Quando a rua não comporta isso, a rua muda: em Mahogany o `map.bin` foi alargado em duas linhas, porque reduzir a cena para caber no mapa antigo era resolver o problema errado.
- **Câmera: a caixa de diálogo come as três linhas de baixo (V27).** A tela tem 15×10 metatiles com o tile da câmera na coluna 7 e na linha 4. Uma cena com atores espalhados verticalmente precisa escolher o centro de modo que essas três linhas caiam em cenário morto. Em Mahogany os dois enquadramentos (`special SpawnCameraObject`) ficam na linha 22 e não na 21 exatamente por isso: a faixa de rocha ao sul da cidade é o que a caixa cobre.
...
- **Convocação do Looker por telefone, uma por dia (V26, pedido do autor + auditoria V3 do roteiro da M1 — padrão do arco daqui para a frente).** Fechar uma missão **não** abre o briefing seguinte. O escritório de Olivine passa a oferecer só uma fala de espera ("we're still checking the reports; I'll call when we have a lead") até o Looker **telefonar**. A ligação é a convocação: ela **nomeia a próxima cidade**, e o briefing em Olivine é onde o plano é apresentado. O gancho de fim de missão continua sem destino — quem revela agora é o telefone, não a despedida, e a despedida pede que o jogador **espere a ligação**.
  **Mecanismo.** `ShouldDoRiftMissionCall` em `src/field_control_avatar.c`, dentro do `TryStartStepCountScript` onde já vivem as ligações do Wally e do Scott, disparando `RiftMissions_EventScript_LookerCall` (`data/scripts/rift_missions.inc`). Quatro condições: estado em {4, 6, 8, 10}; `FLAG_DAILY_LOOKER_CALL` limpa (daily flag — o `ClearDailyFlags` a limpa na virada da **data do calendário do jogo**, não 24 h depois da última ligação); `FLAG_RIFT_LOOKER_SUMMONS` limpa (convite já emitido nunca é reemitido: ele **persiste entre dias e saves** e espera o jogador); e nenhuma pendência da missão anterior — hoje isso é o Type: Null que o Gladion pode ainda estar segurando. Nunca dentro da própria casa: estar lá **adia** a ligação, não a consome. Um retry depois de blackout não muda estado e portanto não toca telefone.
  **O estado 2 está fora da lista de propósito.** A convocação de Blackthorn é a cena de ligação em New Bark; aquele convite "persiste até o briefing, sem expirar nem produzir lembretes nos dias seguintes". E submeter o 2 à flag de convite travaria qualquer save feito antes de a flag existir.
  **Todo script que fecha uma missão seta `FLAG_DAILY_LOOKER_CALL`**, porque nos cinco casos o Looker acabou de falar com o jogador na rua: terminar Blackthorn no dia em que ele ligou não compra uma segunda ligação naquele dia. O telefone é a convocação **seguinte**, não um eco do que acabou de ser dito.
  **Implementado para os quatro estados de uma vez.** O que cada missão ainda tem de fazer por conta própria é a despedida: a da M1 pede para esperar a ligação; as da M2, M3 e M4 ainda mandam o jogador a Olivine na hora.
- **Checkpoint de retry (V26).** Uma missão de ameaça pode ser perdida, e o blackout **recarrega o mapa**: todo `FLAG_TEMP_*` morre. Sem um estado persistente, a segunda tentativa reapresenta tudo — o resgate de surpresa, o reconhecimento do Líder local, a identificação da criatura — como se ninguém se conhecesse. A M1 passa a ter **uma flag persistente própria para isso** (`FLAG_BLACKTHORN_UB_ENGAGED`), setada no último beat antes da escolha e limpa no fim da missão. Ela faz duas coisas: o `ON_TRANSITION` repõe as UBs e o acompanhante **nos tiles de combate** (`setobjectxyperm`, que é grátis porque os templates são recarregados a cada load), e o NPC de entrada usa **texto de retry** em vez de cumprimento. O retry passa a ser "vocês estão segurando, eu estou pronto de novo" e cai direto na escolha da UB. **Implementado na M1 e na M2; M3 e M4 ainda reapresentam a cena inteira.** A M2 leva o padrão um passo adiante (V27): ela tem **duas** flags persistentes, porque tem duas cenas para não repetir. `FLAG_MAHOGANY_UB_ENGAGED` evita as apresentações; `FLAG_MAHOGANY_UB_SAW_RECHARGE` decide se a recarga ainda é descoberta ou se o plano já é o curto, e é ela que põe a parede de gelo de volta no chão no `ON_LOAD` para que a tentativa nova comece **mostrando** a UB rompendo um trecho dela. Uma terceira, `FLAG_MAHOGANY_UB_PICKED_CELESTEELA`, existe só porque esse beat precisa saber qual UB o jogador enfrentava, e `VAR_TEMP_3` morre no blackout.
- **Presente que falha não apaga a vitória (V26).** A checagem de equipe + PC cheios antes do SIM torna a falha de entrega inalcançável, mas se ela acontecer o certo **não** é refazer a batalha. A M1 separa os dois resultados: a cidade fica salva (flag do incidente limpa, estado avançado, moradores de volta) e só o presente fica pendente, numa flag própria (`FLAG_BLACKTHORN_TYPE_NULL_PENDING`) que mantém **apenas** quem dá o presente na rua, com um diálogo que conclui só a entrega. **Implementado na M1;** vale como forma para qualquer presente de missão futura.
- **Tentativa de contenção (V26, roteiro V2 da M1).** Depois da vitória, antes da absorção, a Anabel **tenta** capturar a UB derrotada com uma Ball do equipamento policial dela — e a ligação com a fenda repele a Ball, intacta. É mostrado, não narrado (movimento de arremesso, som de barreira, tremor direcional da fenda, som da Ball quicando), nada sai da bolsa do jogador e não há batalha de captura. A dedução da Anabel é **local** ("essa ainda está ligada à fenda"), nunca uma lei universal sobre Poké Balls. É de onde sai o objetivo da M2 — quebrar a ligação — e é o motivo pelo qual a barreira de gelo da Lillie responde a um problema que o jogador já viu acontecer. **Implementado na M1.**
- **Parceiro da história aparece antes de ser comentado (V26).** Qualquer fala que descreva a reação de um Pokémon do jogador exige que ele esteja **na tela** primeiro. A M1 usa quatro templates de object event no mesmo tile (Cosmog, Cosmoem, Solgaleo, Lunala) partilhando **uma** `FLAG_TEMP_*`: no máximo um é adicionado, e o `removeobject` esconde os quatro. O follower comum fica escondido pela cena inteira, então nada é duplicado. O movimento tem de ser verdadeiro para o sprite que está na tela — Cosmoem flutua e não anda, então o recuo dele é no lugar. Mesmo padrão já usado no altar (§8) para Solgaleo/Lunala.
- **Trilha própria durante a ruptura (V23, pedido do autor).** O tema da cidade tocando enquanto o Necrozma rasga o céu estraga a cena ("fica bizarra a música feliz"). Receita: `fadeoutbgm 4` quando alguém percebe (silêncio no tremor), `playbgm MUS_DP_LEGEND_APPEARS, TRUE` quando a criatura aparece, `fadedefaultbgm` no pós-cena. O `TRUE` grava em `savedMusic`, e é o que faz a batalha de boss devolver a trilha certa ao voltar; carregar mapa limpa o `savedMusic` sozinho, então o retry começa limpo. A trilha é a mesma em todas as missões — ela é **do Necrozma**, não da cidade. **Implementada na M1 (V26) e na M3; M2 e M4 pendentes.** Na M1 o `playbgm` é repetido na entrada do retry, porque um retry começa depois de um carregamento de mapa e o `savedMusic` foi zerado: sem isso o tema alegre da cidade tocaria por cima da luta.
...
## 29. Registro da revisão V26 — o roteiro V2 da Missão 1 e a ligação diária do Looker (25/09/2026)

Duas entregas, uma pedida por documento e outra pedida em uma frase.

### 29.1. O roteiro V2 da Missão 1

O autor entregou `BLACKTHORN_ULTRABEAST_SCRIPT_V2.md`: uma reescrita completa das
falas da Missão 1 mais um conjunto de contratos de cena, para ser **implementada**,
não discutida. Foi implementada inteira. Pedido por pedido:

| Pedido do roteiro V2 | Como ficou |
| --- | --- |
| Falas mais curtas e um falante por caixa | Os textos monolíticos com quatro plaquinhas viraram uma caixa por falante. O briefing da M1 virou **cinco** `msgbox` seguidos, sem `closemessage` entre eles, cada um começando com o seu `{SPEAKER ...}` |
| Ligação do Looker cordial e direta, sem a cobertura das férias | `NewBarkTown_Text_LookerCall` reescrito: parabéns, nome, pedido, endereço. Sai a autocorreção teatral tripla |
| O ataque da Clair tem de **ter efeito visível** | `Kingdra! Dragon Pulse!` é uma caixa própria; o Necrozma recua de (14,50) para (13,50) com o olhar travado, pulsa e volta. Ela contém, não resolve — e não perdeu uma hora batendo em nada |
| A fenda precisa de **borda visível** durante a absorção e a reação do parceiro | Objeto novo `LOCALID_BLACKTHORN_UB_RIFT` (`OBJ_EVENT_GFX_ALTAR_RIFT`, 32x32, o mesmo do altar) em (14,51), ao lado do Necrozma. Aberto quando ele rasga, fechado atrás dele |
| As UBs não podem parecer uma formação | Dois movimentos diferentes: Buzzwole em `walk_fast_right` ×3 pela linha reta; Pheromosa com `delay_16`, um amago para o lado e `walk_faster_right` ×3 pela lateral. Ver a ressalva da regra de sinergia no §6 |
| O resgate é frente **e depois** lateral | O Silvally bate para oeste (o Buzzwole recua), vira sul e bate de novo (a Pheromosa perde a abertura). Sequencial, não um golpe que derruba as duas |
| O Gladion **nomeia** o Necrozma | `UBNecrozmaNamed`. A regra antiga do §6 foi reescrita, não duplicada |
| Tentativa de contenção antes da absorção | `UBContain`: a Anabel sobe para (26,51), arremessa, a ligação repele a Ball. Regra comum nova no §6 |
| A absorção é tentada, não assistida | Kingdra e Silvally atacam a ligação antes de as UBs sumirem; elas dão um passo contra o arrasto antes de serem levadas |
| Oscilação da fenda **obrigatória em todos os ramos** | Depois do flash de ganho de luz, sempre: `UBRiftPulse` (tremor curto, sem flash) + `SE_WARP_IN`. Não depende de família Cosmog na equipe |
| O parceiro **aparece** antes de qualquer fala sobre ele | Quatro templates em (19,51) com `FLAG_TEMP_5`; um é adicionado, cry certo, e só então a fala. Regra comum nova no §6 |
| O Type: Null do presente é outro indivíduo | Objeto próprio em (19,49). O Silvally abre (19,50) indo para (18,50), então os dois ficam na tela juntos |
| Type: Null vai por iniciativa própria | Ele desce para (19,50) e vira para o jogador; o Gladion não dá ordem, e a fala dele é de quem está surpreso |
| Retry com texto próprio e sem reapresentar ninguém | `FLAG_BLACKTHORN_UB_ENGAGED`. Regra comum nova no §6 |
| Falha de entrega não repete a batalha | `FLAG_BLACKTHORN_TYPE_NULL_PENDING`. Regra comum nova no §6 |
| Resultado sem vitória não diz que elas fugiram | `UBUnresolved`: "They're still here. Regroup before we try again." |
| O briefing da M2 tem de assumir o que a M1 revelou | `Text_BriefingM2` agora chama o Necrozma pelo nome e já diz o que a Anabel procura: cortar a ligação |
| Bilhete da porta trancada mais curto | Só o bilhete. A caixa de narração "the door is locked tight" saiu — o som da porta já diz isso |
| Balanceamento | **Intocado**: 3 barras / Lv75 / x120 / moveset curado + item |

**O que o roteiro V2 mandou remover, e foi removido:** os comentários opcionais da
Anabel e do Necrozma sobre a família Cosmog **antes** da cena. A reação passou a
ser um único momento, depois da absorção, com o Pokémon de verdade na tela. Evita
dizer duas vezes a mesma advertência e evita a primeira observação acontecer com
o follower escondido.

**A contradição que o V2 abre, e que este documento resolve de um lado só:** com
o Necrozma nomeado em Blackthorn, a regra "ninguém o nomeia até Olivine" morre.
O §6 foi reescrito. O briefing da M2 foi corrigido junto, porque é uma fala que
acontece **depois** da M1 e diria uma bobagem. As falas de **cena** da M2, M3 e
M4 continuam tratando o nome como novidade em alguns pontos: isso é um passe de
diálogo pendente, não um contrato, e não foi feito aqui porque o autor não pediu
para mexer nas outras missões.

### 29.2. A convocação diária do Looker

Pedido do autor, em uma frase: *"agora o Looker vai te ligar para te chamar
Olivine antes de iniciar Mahogany, e ele só te liga uma vez por dia (e aí vai ser
esse pattern aqui em diante se já quiser implementar)"*. A **auditoria V3** do
roteiro da M1 (§4 e §13.3 dele) foi mais longe e transformou a ligação em **gate
do briefing**, não em lembrete: antes dela, a casa em Olivine não encena nada.

Implementado assim, para os quatro estados de uma vez. Mecanismo, condições e a
razão de o estado 2 ficar fora estão na regra comum do §6.

Duas decisões que o pedido não fixava:

- **A ligação nomeia a cidade; o briefing dá o plano.** É o que o §12 do roteiro
  manda ("a próxima localização será anunciada na ligação de convocação"). O
  gancho de fim de missão continua sem destino, então a regra "gancho sem destino"
  da V19 continua valendo — só mudou quem faz a revelação.
- **O fim de cada missão gasta a ligação do dia.** O Looker acabou de pedir em
  pessoa que o jogador espere o telefone; ligar um passo depois seria ridículo.
  Quem espera um dia recebe a ligação; quem vai direto a Olivine encontra a fala
  de espera, que é o comportamento certo.

### 29.3. O que continua em aberto

- **Runtime da M1 inteira, de novo.** A cena mudou muito para herdar o "testei
  tudo, ficou perfeito" de 22/09/2026. A lista está no §11 do doc da M1.
- **Passe de diálogo na M2, M3 e M4** para a identidade do Necrozma deixar de ser
  tratada como segredo em cena (§29.1).
- **Trilha da ruptura na M2 e na M4** (a M1 e a M3 já têm).
- **Checkpoint de retry na M2, M3 e M4**: hoje as três reapresentam a cena
  inteira depois de um blackout.
- **A daily flag do Looker e o clima:** a ligação toca em qualquer mapa, inclusive
  dentro de prédios e cavernas. Se isso incomodar em runtime, o filtro certo é o
  mesmo `MapAllowsMatchCall` que o Match Call usa, e é uma linha.
- **Despedida das outras três missões.** A da M1 pede para esperar a ligação; a da
  M2, da M3 e da M4 continuam mandando o jogador a Olivine na hora, o que agora
  contradiz o que a casa responde. É uma caixa de texto por missão.
- **Acoplamento da predicate.** `ShouldDoRiftMissionCall` cita
  `FLAG_BLACKTHORN_TYPE_NULL_PENDING` por nome. A segunda missão que puder
  terminar com presente pendente tem de acrescentar a sua linha ali; se chegar a
  três, vale uma flag genérica de "pendência de missão".
