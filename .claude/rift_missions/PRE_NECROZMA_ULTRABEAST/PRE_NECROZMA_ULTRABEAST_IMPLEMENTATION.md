...
**Roteiro da cena (falas e movimentos):** [`PRE_NECROZMA_ULTRABEAST_SCRIPT.md`](PRE_NECROZMA_ULTRABEAST_SCRIPT.md)
...
[`CHERRYGROVE_ULTRABEAST_IMPLEMENTATION.md`](../CHERRYGROVE_ULTRABEAST/CHERRYGROVE_ULTRABEAST_IMPLEMENTATION.md) (M3) e
[`NEWBARK_ULTRABEAST_IMPLEMENTATION.md`](../NEWBARK_ULTRABEAST/NEWBARK_ULTRABEAST_IMPLEMENTATION.md) (M4).
...
**Beats e falas:** [`PRE_NECROZMA_ULTRABEAST_SCRIPT.md`](PRE_NECROZMA_ULTRABEAST_SCRIPT.md), "Ato 2".

Contrato técnico deste bloco (`OlivineCity_House1_EventScript_ReunionScene`):

- A Lillie (3,6) e o Gladion (3,7) estão na **mesma coluna**, um tile de
  distância, então dá para eles olharem **um para o outro**: y cresce para baixo,
  logo Lillie `DIR_SOUTH` e Gladion `DIR_NORTH`. Depois os dois voltam para
  `DIR_EAST`, encarando a sala e a mãe.
- O Kukui já está olhando para o oeste (para dentro da sala): **não** precisa de
  `turnobject`.
- A Anabel está em (5,7) e a Lusamine em (9,6): ela vira `DIR_EAST` para falar
  com ela e volta para `DIR_SOUTH`.
...
Todas as falas da reunião — a cena longa, a pergunta, os dois desfechos, o passe
da balsa, as conversas soltas do estado 11 e as do pós-jogo — estão em [`PRE_NECROZMA_ULTRABEAST_SCRIPT.md`](PRE_NECROZMA_ULTRABEAST_SCRIPT.md).

Regras que qualquer reescrita precisa respeitar:

- Vozes do design §3.1: Looker teatral e caloroso; Anabel precisa; Lusamine
  formal e sem rodeio; Lillie direta; Gladion econômico; Kukui entusiasmado.
- **Sem travessão `—`**: U+2014 não está em `charmap.txt` e quebra o build
  (M4 §12.7). `“ ”` e `…` podem.
- A conversa de família **abre** aqui e **não** se resolve aqui (design §7).
- A condição da Anabel é **um argumento, não novidade**: ela já disse isso em
  New Bark. Uma caixa de retomada, não mais.
